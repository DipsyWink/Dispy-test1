...
void publicarResultado() {
  ...
  String json = "{\"tag\": \"" + String(tag) + "\", \"score\": " + String(nivel) + "}";
  
  char attributes[100];
  json.toCharArray(attributes, 100);
  
  client.publish("disruptive/genius/rank", attributes);
  Serial.println("Resultado publicado: " + json);
}
...
void runTagSelection() {
  lcd.setCursor(0, 0); lcd.print("Escolha sua TAG:");
  lcd.setCursor(5, 1);
  for (int i = 0; i < 3; i++) {
    if (i == tagIndex) { lcd.print("["); lcd.print(tag[i]); lcd.print("]"); }
    else { lcd.print(" "); lcd.print(tag[i]); lcd.print(" "); }
  }
  ...
  if (btn != 0 && keyDown == 0) {
    keyDown = 1;
    tone(buzzer, 1000, 50);
    if (btn == 1) { tag[tagIndex]++; if (tag[tagIndex] > 'Z') tag[tagIndex] = 'A'; }
    else if (btn == 2) { tag[tagIndex]--; if (tag[tagIndex] < 'A') tag[tagIndex] = 'Z'; }
    else if (btn == 3) { if (tagIndex > 0) tagIndex--; }
    else if (btn == 4) {
      tagIndex++;
      if (tagIndex >= 3) {
        lcd.clear(); lcd.print("Boa sorte, "); lcd.print(tag);
...
void runGameOver() {
  ...
  lcd.setCursor(1, 1); lcd.print(tag); lcd.print(" Score: "); lcd.print(nivel);
  ...
  nivel = 0; atual = 0; GameState = 0; tagIndex = 0;
...
void loop() {
  if (!client.connected() && estadoAtual != STARTUP) {
    reconnect();
  }
  client.loop();
